...
class Battle::Scene::PokemonDataBox
  ...
  # X-Position for Singles and Doubles Battles
  SINGLES_X_POSITION = 102
  DOUBLES_X_POSITION = 132
...
class Battle::Scene::PokemonDataBox
  ...
  def initializeOtherGraphics(*args)
    ...
    when 2
      ...
      # Check if it's a Singles or Doubles battle
      is_doubles = @battler.battle.pbSideSize(0) > 1 || @battler.battle.pbSideSize(1) > 1
      x_position = is_doubles ? DOUBLES_X_POSITION : SINGLES_X_POSITION
      
      # Use absolute positioning based on battle type
      if @battler.opposes?(0)
        # Opponent - Position from the right
        @types_x = @databoxBitmap.width - @types_bitmap.width - x_position
      else
        # Player - Position from the left
        @types_x = x_position
      end
      @types_y = 4
    when 1
