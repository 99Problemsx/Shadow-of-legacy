...
class Sprite_WaterBubble
  ...
  def load_water_texture
    texture_path = "Graphics/Pictures/Footset/splash.png" # Change to Location of image path. If using splashGBA change from splash.png to splashGBA.png
  ...
  def update
    ...
    if @event.character_name.empty? || @event.character_name == "nil" || 
       $game_map.terrain_tag(@event.x, @event.y) != 18 || @event.transparent # Change != 18 to the tile # used in game.
      if @sprite
        @sprite.dispose
        @sprite = nil
      end
    ...
    return if disposed? || !$scene || !$scene.is_a?(Scene_Map)
    if @event.character_name.empty? || @event.character_name == "nil" || 
      $game_map.terrain_tag(@event.x, @event.y) != 18 || @event.transparent ||
