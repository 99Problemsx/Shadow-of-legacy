...
class Sprite_Character
  ...
  def update
    ...
    if (@character.x != @old_x || @character.y != @old_y) && !["", "nil"].include?(@character.character_name)
      # Vereinfachte Logik: Prüfe einfach das Terrain Tag
      if (!@character.respond_to?(:name) || !EVENTNAME_MAY_NOT_INCLUDE.include?(@character.name)) &&
        ...
        make_steps = [2,1,0].any? do |e|
          ...
          terrain_tag = $data_tilesets[tilesetid].terrain_tags[tile_id]
      ...
      if make_steps
        ...
        begin
          if @character == $game_player && $PokemonGlobal.bicycle
            filename = "Graphics/Characters/steps#{dirs[@character.direction]}Bike"
            fstep.bitmap = AnimatedBitmap.new(filename).bitmap
          else
            filename = "Graphics/Characters/steps#{dirs[@character.direction]}"
            fstep.bitmap = AnimatedBitmap.new(filename).bitmap
          end
        rescue => e
          fstep.dispose
          # Überspringe die Fußspur-Erstellung wenn Fehler auftritt
          @old_x = @character.x
          @old_y = @character.y
          update_footsteps
          return
