...
module BoxAutoSort
  ...
  def self.show_sort_menu(storage, current_box = nil)
    ...
    when 7 then :CATCH_DATE  # Same method, direction is fixed in algorithm
    when 8 then :SHINY_FIRST
    when 9 then :SPECIES_ID
    when 10 then :FORM_ID
    when 11 then :FRIENDSHIP
    when 12 then :NATURE
    end
